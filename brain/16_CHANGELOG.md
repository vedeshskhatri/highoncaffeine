...
[2026-09-11 21:30] AUDIT REMEDIATION — Complete excision of fabricated ANSYS validation claims across repository (JUDGE_FEEDBACK_AUDIT.md critical #1). Deleted synthetic result files and plots (case1_comparison.json, synthetic_case*.csv, case*_overlay.png). Renamed compare.py flag to --synthetic-selftest with mandatory ASCII disclosure banner and refusal to write mock result files. Purged fabricated Delta-T / RMSE values and badges from web components (ValidationSection, ValidationPanel, DataProvenancePanel, VerifyPage, MethodPage), engine/report.py, engine/provenance.py, README.md, ANSYS_REFERENCE.md, 10_VALIDATION.md, and 08_UI_SPEC.md. Added verbatim honest disclosure on /method stating reference cases and comparison harness are built, workstation runs are pending, and current validation is grounded in published DIHAR empirical data.
```
